...
public:
    ...
    {
        // tele: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // tele: get transaction coin age
...
public:
    ...
    // tele: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // tele: two types of block: proof-of-work or proof-of-stake
    ...
    // tele: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // tele: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // tele: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // tele: block index flags
    enum  
    {
